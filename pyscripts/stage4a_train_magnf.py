...
def forward(data, model, config):
    ...
    estimate = model(coord[:, 0], coord[:, 1], sidx)

    loss_mse = mse_nandetect(mag_oracle, estimate)
    loss_value = config.mag_weight * loss_mse
    return loss_value
...
def main():
    ...
    for epoch in range(max_epoch):
        ...
        for data in tqdm(tr_data_loader):
            ...
            model.zero_grad()
            loss_value = forward(data, model, config.loss)
            loss_value.backward()
